...
func TestParserSmall(t *testing.T) {
	...
	// fmt.Println(p.root.String())
}

func TestParserMedium(t *testing.T) {
	lx := lexer.NewLexer(bufio.NewReader((strings.NewReader(LoadMedium))))
	p := NewParser(lx)
	err := p.Parse()
	if err != nil {
		t.Errorf("unexpected: %s", err.Error())
...
func TestParserLarge(t *testing.T) {
	lx := lexer.NewLexer(bufio.NewReader((strings.NewReader(LoadLarge))))
	p := NewParser(lx)
	err := p.Parse()
	if err != nil {
		t.Errorf("unexpected: %s", err.Error())
...
func BenchmarkParserSmall(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			rd := bufio.NewReader(strings.NewReader(LoadSmall))
			l := lexer.NewLexer(rd)
			p := NewParser(l)
			err := p.Parse()
			if err != nil {
...
func BenchmarkParserNoParallelSmall(b *testing.B) {
	b.ReportAllocs()
	b.SetBytes(int64(len(LoadSmall)))
	b.ResetTimer()
	for range b.N {
...
func BenchmarkParserMedium(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			rd := bufio.NewReader(strings.NewReader(LoadMedium))
			l := lexer.NewLexer(rd)
			p := NewParser(l)
			err := p.Parse()
			if err != nil {
...
func BenchmarkParserNoParallelMedium(b *testing.B) {
	b.ReportAllocs()
	b.SetBytes(int64(len(LoadMedium)))
	b.ResetTimer()
	for range b.N {
...
func BenchmarkParserLarge(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			rd := bufio.NewReader(strings.NewReader(LoadLarge))
			l := lexer.NewLexer(rd)
			p := NewParser(l)
			err := p.Parse()
			if err != nil {
